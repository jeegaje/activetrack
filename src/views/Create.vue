<template>
  <div class="max-w-screen-md mx-auto px-4 py-10">
    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      <!-- Form -->
      <form @submit.prevent="workoutCreate" class="flex flex-col gap-y-5 w-full">
        ...
        <div class="flex flex-col">
          ...
          <select id="workout-type" class="p-2 text-grey-500 focus:outline-none" required v-model="workoutType" @change="workoutChange">
        ...
        <div v-if="workoutType == 'strength'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            ...
            <img src="@/assets/images/trash-light-green.png" class="h-4 w-auto absolute -left-5 cursor-pointer" alt="" @click="workoutDelete">
          </div>
        ...
        <div v-if="workoutType == 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            ...
            <img src="@/assets/images/trash-light-green.png" class="h-4 w-auto absolute -left-5 cursor-pointer" alt="" @click="workoutDelete(item.id)">
          </div>
          <button type="button" @click="addExercise">Add exercise</button>
        </div>
...
export default {
  ...
  setup() {
    ...
    const workoutDelete = (id) => {
      ...
      errorMsg.value = "Exercise must more than 1"
      setTimeout(() => {
    ...
    const workoutCreate = async() => {
      try{
        const { error } = await supabase.from('workouts').insert([
          {
            ...
            user_id: store.methods.getUserId()
          }
        ])
        ...
        exercises.value = [];
        statusMsg.value = "Success create workout"
        setTimeout(() => {
    ...
    return {workoutName, workoutType, exercises, statusMsg, errorMsg, addExercise, workoutChange, workoutDelete, workoutCreate};
  },
};
</script>
